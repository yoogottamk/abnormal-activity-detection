...
if [ ! -e "$FILE_DOWN" ]; then
	if ! wget -qO- "localhost:9999/test/" >/dev/null 2>&1; then
		...
		wget -qO- --header="Content-Type: application/json" --post-data="{\"chat_id\": \"$id\", \"parse_mode\": \"Markdown\", \"text\": \"*ALERT!* The Flask server seems to be down.\"}" $BOT_URL > /dev/null 2>&1
	    done
